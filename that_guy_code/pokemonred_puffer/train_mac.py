...
import wandb
import yaml
from that_guy_code.pokemonred_puffer import eval
from that_guy_code.pokemonred_puffer.cleanrl_puffer import CleanPuffeRL, rollout
from that_guy_code.pokemonred_puffer.env_creator import env_creator
...
warnings.filterwarnings("ignore", message="The emulation speed might not be accurate when speed-target is higher than 5")
...
def make_policy(env, env_module, args):
    ...
    policy = policy.to(device, non_blocking=True)  # Use the dynamically determined device
...
if __name__ == "__main__":
    ...
    parser.add_argument("--yaml", default="that_guy_code/config.yaml", help="Configuration file to use")
    parser.add_argument(
    ...
    parser.add_argument("--rom-path", default="storage/PokemonRed.gb")#red.gb
    parser.add_argument("--state-path", default="that_guy_code/pyboy_states/Bulbasaur.state")
    ...
    env_module = importlib.import_module(f"that_guy_code.pokemonred_puffer")
