...
import numpy as np
from src.red_types import PlayerState, Reward, History
from src.device import DeviceHandler

from src.globals import output_shape, frame_stacks, memory_height
...
class GameHandler:
    ...
    def get_money(self) -> int:
        return (100 * 100 * self.read_bcd(self.devicehandler.read_m(0xD347)) + 
    ...
    def read_triple(self, start_add: int) -> int:
        return 256*256*self.devicehandler.read_m(start_add) + 256*self.devicehandler.read_m(start_add+1) + self.devicehandler.read_m(start_add+2)
    ...
    def get_pos(self) -> dict:
        return {"x": self.devicehandler.read_m(0xD362), "y": self.devicehandler.read_m(0xD361), "map_n": self.devicehandler.read_m(0xD35E)}
    
    def get_opponent_level(self) -> int:
        opponent_level = max([self.devicehandler.read_m(a) for a in [0xD8C5, 0xD8F1, 0xD91D, 0xD949, 0xD975, 0xD9A1]]) - 5
