...
from src.red_types import Reward, History
...
class RewardHandler:
    ...
    def get_exploration_reward(self, history: History,gamehandler: GameHandler):
        position = gamehandler.get_pos()
        x_pos = position["x"]
        y_pos = position["y"]
        map_n = position["map_n"]
        coord_string = f"x:{x_pos} y:{y_pos} m:{map_n}"
        self.seen_coords_relative_len = len(history.seen_coords[coord_string]) / np.median([len(v) for v in history.seen_coords.values()])
        return self.seen_coords_relative_len
    ...
    def compute_reward(self, history: History, gamehandler: GameHandler):
        ...
        exploration_reward = self.get_exploration_reward(history,gamehandler)
